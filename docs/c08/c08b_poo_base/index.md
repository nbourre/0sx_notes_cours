...
Vous pouvez imaginer un chien noir qui a une certaine grandeur avec un pelage court. Il peut courir, aboyer, marcher, etc.

Une classe décrit les propriétés et les comportements d'un objet générique.

Ainsi dans le cas d'un chien, nous avons les propriétés suivantes couleur, taille, pelage, etc. et les comportements suivants courir, aboyer, marcher, etc.
...
On pourrait dire que "Idéfix" est un chien blanc, de taille petite, avec un pelage court. Il peut courir, aboyer, marcher, etc.
...
Plusieurs langages de programmation modernes n'utilisent qu'un seul fichier pour définir une classe comme le C#. Cependant, **par convention** en C++, on utilise deux fichiers pour définir une classe. Un fichier `.cpp` pour la définition de la classe et un fichier `.h` pour la déclaration de la classe.
...
On démarre un bloc public avec le mot-clé `public` et la partie privée avec le mot-clé `private`.
